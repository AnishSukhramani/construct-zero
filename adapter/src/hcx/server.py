"""FastAPI OpenAI-compatible server for HCX."""
...
from hcx import __version__
from hcx.config import HCXConfig, load_config
from hcx.core.backend import CompletionResult
from hcx.core.sessions import SessionStore
from hcx.core.streaming import sse_line
from hcx.drivers.claude_code import ClaudeCodeDriver
from hcx.drivers.cursor import CursorDriver
from hcx.openai_types import (
...
def create_app(config: HCXConfig | None = None) -> FastAPI:
    config = config or load_config()
    sessions = SessionStore()
    backend = _build_backend(config, sessions)

    app = FastAPI(title="HCX Adapter", version=__version__)
    ...
    def health(request: Request) -> HealthResponse:
        ...
        cfg: HCXConfig = request.app.state.config
        status = b.health()
...
def _build_backend(config: HCXConfig, sessions: SessionStore):
    backend_name = (config.inference.backend or "cursor").strip().lower()
    if backend_name in {"cursor", "hcx"}:
...
def _stream_response(backend, body: ChatCompletionRequest) -> Iterator[str]:
    try:
        ...
        yield sse_line({"error": {"message": str(exc), "type": "hcx_error"}})
        yield sse_line("[DONE]")
