...
from hcx.config import CursorDriverConfig, HCXConfig
...
from hcx.core.streaming import completion_id
from hcx.drivers.acp import check_acp
...
class CursorDriver:
    ...
    def __init__(self, config: HCXConfig, sessions: SessionStore | None = None) -> None:
        self.config = config
    ...
    def _make_workspace(self) -> str:
        if self.cursor_cfg.workspace_isolation:
            return tempfile.mkdtemp(prefix="hcx-cursor-ws-")
        return os.getcwd()
            ...
            if active.batch_event.wait(timeout=0.2):
                ...
                if tools:
                    ...
                    return CompletionResult(
                        ...
                        model=active.model,
                        raw={"hcx_session_id": active.session.session_id},
                    )
    ...
    def _start_tool_run(
        ...
        threading.Thread(
            target=runner,
            name=f"hcx-cursor-{session.session_id}",
            daemon=True,
        ).start()
