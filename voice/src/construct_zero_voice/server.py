...
from construct_zero_voice.hermes_bridge import ask_hermes, check_adapter_up
from construct_zero_voice.session_store import SessionStore
from construct_zero_voice.stt_whisper import WhisperSTT
from construct_zero_voice.tts_kokoro import KokoroTTS
...
async def lifespan(app: FastAPI):
    preload = (env_cz("VOICE_PRELOAD") or "").strip() in ("1", "true", "yes")
    if preload:
        try:
            get_stt().load()
        except Exception:
...
app = FastAPI(title="Construct-Zero Voice", version=__version__, lifespan=lifespan)
...
def health() -> dict[str, Any]:
    stt = get_stt()
    tts = get_tts()
    adapter_ok, adapter_detail = check_adapter_up()
    hermes_ok = False
    hermes_detail = "not checked"
    try:
        from construct_zero_voice.hermes_bridge import default_hermes_bin
    ...
    adapter_info = {"ok": adapter_ok, "detail": adapter_detail}
    return {
        "status": status,
        "version": __version__,
        ...
        "hcx": adapter_info,  # deprecated alias
...
def run(host: str | None = None, port: int | None = None) -> None:
    ...
    host = host or env_cz("VOICE_HOST") or "127.0.0.1"
    port = port or int(env_cz("VOICE_PORT") or "8767")
    allow_public = (env_cz("VOICE_ALLOW_PUBLIC") or "").strip() in ("1", "true")
    if host not in ("127.0.0.1", "localhost", "::1") and not allow_public:
        raise SystemExit(
            f"Refusing to bind {host!r} (loopback only). "
            "Set CZ_VOICE_ALLOW_PUBLIC=1 only behind your own reverse proxy / tunnel."
        )
    uvicorn.run(
        "construct_zero_voice.server:app",
        host=host,
        port=port,
        reload=False,
        log_level="info",
