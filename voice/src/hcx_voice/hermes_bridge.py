...
def repo_root() -> Path:
    env = os.environ.get("HCX_REPO_ROOT", "").strip()
    if env:
        return Path(env)
    ...
    for p in [here.parent, *here.parents]:
        if (p / "scripts" / "start-voice.sh").is_file() and (p / "voice").is_dir():
            return p
    # Editable src layout: voice/src/hcx_voice/…
    return here.parents[3]


def default_hermes_bin() -> Path:
    env = os.environ.get("HCX_HERMES_BIN", "").strip()
    if env:
        return Path(env)
    root = repo_root()
...
def hcx_health_url() -> str:
    return os.environ.get("HCX_HEALTH_URL", "http://127.0.0.1:8765/health").strip()


def check_hcx_up(timeout: float = 2.0) -> tuple[bool, str]:
    url = hcx_health_url()
    try:
        r = httpx.get(url, timeout=timeout)
        if r.status_code != 200:
            return False, f"HCX health HTTP {r.status_code}"
        data = r.json()
        if data.get("status") != "ok":
            return False, data.get("detail") or "HCX not ok"
        return True, "ok"
    except Exception as exc:
        return False, f"HCX unreachable: {exc}"
...
def ask_hermes(
    ...
    *,
    ...
    ok, detail = check_hcx_up()
    if not ok:
        raise RuntimeError(detail)

    hermes_bin = default_hermes_bin()
    timeout = timeout or float(os.environ.get("HCX_VOICE_HERMES_TIMEOUT", "180"))
    cmd = [
        str(hermes_bin),
        "chat",
        "-q",
    ...
    env = os.environ.copy()
    # Ensure placeholder HCX key if Hermes requires it
    env.setdefault("HCX_API_KEY", "unused")
